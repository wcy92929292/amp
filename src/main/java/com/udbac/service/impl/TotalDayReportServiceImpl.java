 ...
 */
@Service
public class TotalDayReportServiceImpl extends TotalServiceImpl implements TotalDayReportService{
	...
	public String monthReportList(String cusCode,Date dateStr,String realName,String actCode,Integer dayIf,Integer isRegion) {
		...
		List<MonthReportModel> monthList = totalDao.listMonth1(cusCode, dateStr, actCode, dayIf);
//		monthList.addAll(totalDao.listMonthBaidu1(cusCode,dateStr,actCode,dayIf));
		String reportPath = getMonthReportPath(monthList, month);
		
		newFile(reportPath);
		initCellStyle();
		
		Map<String, MonthMediaSumList> groupMedia = groupMedia(monthList);
		writeMonthDataToXlsx(groupMedia,isRegion,realName,month);
//		setSheetStyle(groupMedia);
		xlsxUtil.close(null);
...
//	private void setSheetStyle(Map<String, TotalMediaSumList> groupMedia) {
//		// TODO Auto-generated method stub
//		Map<String,XSSFCellStyle> cs = groupMedia ? sumCellStyles : cellStyles; 
...
//	}
	...
	public void initMonthReport(){
		//左上角信息
		XSSFCellStyle leftTitle = xlsxUtil.createCellStyle(WHITE, null, SONG_TI, FONT_9, null, XlsxUtil.FONT_BOLD, XlsxUtil.ALIGN_CENTER, XlsxUtil.ALIGN_VERTICAL_CENTER, XlsxUtil.BORDER_THIN,null);
	 ...
	 * @return
	 */
	public int writeMonthDataToXlsx(Map<String, MonthMediaSumList> groupMedia,Integer isRegion,String realName,String dateStr){
			...
			while (iterator.hasNext()) {
				...
				for (int i = 0; i < micsList.size(); i++) {
					...
					if(groupid != 0 && groupid != mics.getGroupId()){
						writeGroupMicsSumData(summics, row, startColumn, isRegion);
						mediaSheetStyle2(row, true);
						row++;
						groupid = 0;
					}
...
//					//媒体中没有合并的点位预估
//					if(!sumList.isMergeByGroupId()){
//						writeMicsSumData(mics, row, startColumn,isRegion);
//						row++;
//					}
...
//					}
				...
				if(!sumList.isMergeByGroupId()){
					...
					writeGroupMeidaSumData(sumList, row, startColumn,isRegion);
				}
				writeLeftTop(sumList, realName, dateStr);
			}//end while
	...
	public int writeMicsData(List<MonthReportModel> monthList,int startRow,int startColumn,String[] methods){
		...
		try {
			for (int i = 0; i < monthList.size(); i++) {
				...
				writeLineData(row, startColumn, model, methods);
				mediaSheetStyle2(row, false);
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	 ...
	 * @return
	 */
	public void writeMicsSumData(Mics mics,int startRow,int startColumn,int isRegion){
//		=SUBTOTAL(9,J81:J116)
...
//		=AVERAGE(V9:V26)
		...
		mediaSheetStyle2(startRow, true);
...
//		AVERAGE(V9:V26)  平均访问时长
	...
	public void writeGroupMeidaSumData(MonthMediaSumList sumList,int startRow,int startColumn,int isRegion){
...
//		=L25+L23+L21+L19+L15+L17+L13+L11+L9
	...
	public void writeLeftTop(MonthMediaSumList sumList,String userName,String dateStr){
		...
		String projectName = reportModel.getCustomerName() +"截至"+
				DateUtil.getDateStr(DateUtil.getDate(dateStr,null) ,"yyyy-MM-dd") + "累计数据";
		String period = "";
		String putDate = DateUtil.getDateStr(new Date(),"yyyy/MM/dd");
		
		period =  "——" + DateUtil.getDateStr(DateUtil.getDate(dateStr,null), "yyyy/MM/dd");
