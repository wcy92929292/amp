...
(function($,win,undef){
	...
	Validform.util={
		dataType:{
			...
			"s500":/^[\u4E00-\u9FA5\uf900-\ufa2d\w\.\s]{0,500}$/, //6到18位字符串
			"p":/^[0-9]{6}$/, // 验证是否为邮政编码
			"m":/^13[0-9]{9}$|14[0-9]{9}|15[0-9]{9}$|18[0-9]{9}|17[0-9]{9}$/, //手机号码格式
			"e":/^\w+([-+.']\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*$/, //email格式
			"url":/^(\w+:\/\/)?\w+(\.\w+)+.*$/, //验证字符串是否为网址
			"idcard":/^\d{15}(\d{2}[A-Za-z0-9])?$/,    //身份证
			"ns" :  /^[0-9a-zA-Z]{2,20}$/,   //字母和数组的组合
			"mob" : /^(0[0-9]{2,3}-)?[0-9]{7,8}$/,
			"ph" : /^(13[0-9]|14[0-9]|15[0-9]|18[0-9]|17[0-9])\d{8}$/,
		...
		check:function(curform,subpost,bool){
			...
			*/
			...
			if(ajaxurl && !Validform.util.isEmpty.call($(this),inputval) && !bool){
				...
				var params= {"param":encodeURIComponent(inputval),
					         "name":encodeURIComponent($(this).attr("name")),
						     "dataDictCode" : $("#dataDictCode").val()
						};
				var localconfig={
					type: "POST",
					cache:false,
