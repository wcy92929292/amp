			...
			function upload(id) {
				...
				if(id == '5') {
					...
					if(file1 != '') {
						...
						$.ajaxFileUpload({
							url: "/amp/admonitor/fileUploadv4.do?date="+$("#sdate_id").val()+'&file=1'+'&format='+$("#format").val(), //submit to UploadFileServlet  
							secureuri: false,
							fileElementId: 'file1',
							...
							success: function(data, status) {
								if(data == 'success') {
									...
									if(file2 != '') {
										$.ajaxFileUpload({
											url: "/amp/admonitor/fileUploadv4.do?date="+$("#sdate_id").val()+'&file=2'+'&format='+$("#format").val(), //submit to UploadFileServlet  
											secureuri: false,
											fileElementId: 'file2',
											dataType: 'text', //or json xml whatever you like~  
											success: function(data, status) {
												if(data == 'success') {
													...
													if(file3 != '') {
														$.ajaxFileUpload({
															url: "/amp/admonitor/fileUploadv4.do?date="+$("#sdate_id").val()+'&file=3'+'&format='+$("#format").val(), //submit to UploadFileServlet  
															secureuri: false,
															fileElementId: 'file3',
															dataType: 'text', //or json xml whatever you like~  
															success: function(data, status) {
																if(data == 'success') {
																	...
																	if(file4 != '') {
																		$.ajaxFileUpload({
																			url: "/amp/admonitor/fileUploadv4.do?date="+$("#sdate_id").val()+'&file=4'+'&format='+$("#format").val(), //submit to UploadFileServlet  
																			secureuri: false,
																			fileElementId: 'file4',
																			dataType: 'text', //or json xml whatever you like~  
																			success: function(data, status) {
		...
		<div>
			<div class="form_box">
				<div id="month" style="text-align: center;">日期:&emsp;
					<input class="text-input text-input-1 day-time" type="text" id="sdate_id" value="<%=edate%>" onfocus="WdatePicker({skin:'twoer'})"/>&nbsp;&nbsp;&nbsp;&nbsp;跳出格式:&nbsp;&nbsp;
					<select id="format" name="">
						<option value="1">短文件(只有仿次指标)</option>
						<option value="2">宽文件(含很多指标)</option>
					</select>&emsp;&emsp;
				</div>
			</div>

			<div class="form_box">
				<div id="month" style="text-align: center;">营销活动访问:&emsp;
				...
				<div id="month" style="text-align: center;">营销活动访客:&emsp;
					<input class="text-input" type="file" id="file2" name="file2" value="" onfocus="cleanRate('2');" />&emsp;&emsp;
				...
				<div style="height:20px;"></div>
				<div id="month" style="text-align: center;">营销活动点击:&emsp;
					<input class="text-input" type="file" id="file3" name="file3" value="" onfocus="cleanRate('3');" />&emsp;&emsp;
				...
				<div id="month" style="text-align: center;">营销活动跳出:&emsp;
					<input class="text-input" type="file" id="file4" name="file4" value="" onfocus="cleanRate('4');" />&emsp;&emsp;
